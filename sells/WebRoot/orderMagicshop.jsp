...
<input type=hidden name='price' value='${item.price}'>${item.itemNo}</td>
		...
		<tr>
          <td height="28" colspan=6 bgcolor=#ffffff> 若您結帳時選擇<span class="style1">貨到付款,超商繳費[7-11,萊爾富,全家]</span>將加收物流處理費︰${requestScope.sells.process}元 </td>
        </tr>
		</c:if>
...
購物總金額為︰
<c:if test="${requestScope.sells.freightTp == '0'}">
${total+requestScope.sells.freightFare}元 (含運費) <BR><c:if test="${requestScope.sells.process > '0'}">若您選擇貨到付款,超商繳費[7-11,萊爾富,全家] 購物總金額為︰${total+requestScope.sells.freightFare+requestScope.sells.process}元 (含運費及貨到付款物流處理費) <BR></c:if>
...
<c:if test="${requestScope.sells.process > '0'}">若您選擇的付款方式：貨到付款,超商繳費[7-11,萊爾富,全家] 購物總金額為︰${total+requestScope.sells.freightFare+requestScope.sells.process}元 (含運費及貨到付款物流處理費) <BR></c:if></c:if>
<c:if test="${totQty >requestScope.sells.nofreightQty}">${total}元 (免運費)<BR></c:if></c:if>
<c:if test="${requestScope.sells.freightTp == '3'}">
<c:if test="${total <= requestScope.sells.nofreightFare}">${total + requestScope.sells.freightFare}元 (含運費)<BR><c:if test="${requestScope.sells.process > '0'}">若您選擇貨到付款 購物總金額為︰${total+requestScope.sells.freightFare+requestScope.sells.process}元 (含運費及貨到付款物流處理費) <BR></c:if></c:if>
<c:if test="${total > requestScope.sells.nofreightFare}">${total}元 (免運費)<BR><c:if test="${requestScope.sells.process > '0'}">若您選擇貨到付款,超商繳費[7-11,萊爾富,全家] 購物總金額為︰${total+requestScope.sells.process}元 (含貨到付款物流處理費) <BR></c:if></c:if>
...
<c:if test="${totQty >requestScope.sells.nofreightQty}">${total}元 (免運費)<BR><c:if test="${requestScope.sells.process > '0'}">若您選擇貨到付款,超商繳費[7-11,萊爾富,全家] 購物總金額為︰${total+requestScope.sells.process}元 (含貨到付款物流處理費) <BR></c:if></c:if></c:if>
</textarea>
  </td>
    </tr>
  </table>
<table width="800" border="0" align="center" cellspacing="1" class="tb1">
    ...
    <tr>
      ...
      <td width="83%" bgcolor="#ffffff" ><span class="login-box">郵遞區號</span>
        ...
        <option value='自家地址'>自家地址</option>
        <option value='公司地址'>公司地址</option>
        <option value='管理員代收'>管理員代收</option>
        <option value='郵局地址'>郵局地址</option>
        <option value='全家超商'>全家超商</option>
        </select>
        <BR>
        [目前提供<font color="#800080">宅配到府</font>、郵局<font color="#800080">存局後領、郵政信箱、全省 全家便利商店</font>]<BR>
<a href="http://www.post.gov.tw/post/internet/Post_All.htm" target="_blank">郵局地址查詢</a>
<a href="http://www.famiport.com.tw/shop.html" target="_blank">全家店號查詢</a> [寄到全家超商只需填寫該家店的店號即可，例：007948]<BR>
<span style="background-color: rgb(255, 255, 0);">
PS：寄到全家超商或郵局地址<font color="#ff0000">無法貨到付款</font>， 如需貨到付款請填上住家或公司地址小舖才會出貨唷。
</span><BR>
</td>
    </tr>
    <tr>
...
&nbsp;</td>
    ...
    <tr>
      <td width="17%" height="28" align="center" bgcolor="#ffffff" >
        <b class="style8">希望送達時段</b></td>
      <td width="83%" bgcolor="#ffffff" >
      <select name="deTime">
      <option value='0830~1200'>0830~1200</option>
      <option value='0830~1200'>1200~1700</option>
      <option value='0830~1200'>1700~1900</option>
      </select>
&nbsp;</td>
    </tr>
    <tr>
      ...
      <td width="83%" bgcolor="#ffffff" ><textarea name="desc" cols="60" rows="10"></textarea>
        &nbsp;<br>
        <span class="style8">1500字以內,如需開收據請於備註告知～～產品若寄出恕不補開唷。 小舖 <a href='http://www.magicshop.com.tw/buy/cv.jpg' target='_blank'>收據範例</a></span></td>
    </tr>
  </table>
  </td>
    </tr>
  ...
  <table width="800" border="0" align="center" cellspacing="1" class="tb1">
    ...
    <tr>
	  ...
	  <c:if test="${requestScope.sells.payType4 == '1' }">
       ...
       <c:if test="${requestScope.sells.payType711 == '1' }"><option value='7-11繳款-需加收處理費：${requestScope.sells.process}元'>7-11繳款-需加收處理費:${requestScope.sells.process}</option></c:if> 
       <c:if test="${requestScope.sells.payTypeHilife == '1' }"><option value='萊爾富繳款-需加收處理費：${requestScope.sells.process}元'>萊爾富繳款-需加收處理費:${requestScope.sells.process}</option></c:if> 
       <c:if test="${requestScope.sells.payTypeFamily == '1' }"><option value='全家繳款-需加收處理費：${requestScope.sells.process}元'>全家繳款-需加收處理費:${requestScope.sells.process}</option></c:if> 
	  			</c:when>
	  ...
	  <option value='貨到付款-需加收處理費：${requestScope.sells.process}元'>貨到付款-需加收處理費:${requestScope.sells.process}元</option>
       ...
       <c:if test="${requestScope.sells.payTypeFamily == '1' }"><option value='全家繳款-需加收處理費：${requestScope.sells.process}元'>全家繳款-需加收處理費:${requestScope.sells.process}</option></c:if> 
				  ...
				  <c:if test="${total > requestScope.sells.nofreightFare}">
				  <c:if test="${requestScope.sells.payType4 == '1' }"><option value='貨到付款'>貨到付款</option></c:if>
	  ...
	  <c:if test="${requestScope.sells.payType4 == '1' || requestScope.sells.payType711 == '1' || requestScope.sells.payTypeHilife == '1' || requestScope.sells.payTypeFamily == '1'}">
      <c:choose>
       <c:when test="${requestScope.sells.process > '0'}">
         <c:choose>
          <c:when test="${requestScope.sells.freightTp == '0'}">
